...
public class DictAspect {
            ...
            for (Object record : pageUtils.getData()) {
                ...
                for (Field field : ObjConvertUtils.getAllFields(record)) {
                    ...
                    if (field.getAnnotation(Dict.class) != null) {//如果该属性上面有@Dict注解，则进行翻译
                        String dimType = field.getAnnotation(Dict.class).dimType();//拿到注解的dictDataSource属性的值
                        String text = field.getAnnotation(Dict.class).dictText();//拿到注解的dictText属性的值
                        //获取当前带翻译的值
                        String key = String.valueOf(item.get(field.getName()));
                        //翻译字典值对应的text值
                        String textValue = translateDictValue(dimType, key);
     ...
     * @return
     */
    private String translateDictValue(String dimType, String key) {
        //如果key为空直接返回就好了
        ...
        for (String k : keys) {
            ...
            tmpValue = dataItemService.selectByDatasourceKey(dimType, k.trim());
